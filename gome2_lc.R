...
add_lc <- function(input_file){
  ...
  elem_dim <- myfile$dim[['obs']]
  
  lc_name  <- paste0("LC_MASK_", year)
  lc_perc  <- paste0("LC_PERC_", year)
...
mclapply(input_list, add_lc, mc.cores = 6, mc.preschedule = FALSE)
